...
export function Facility({ className, children }: FacilityProps) {
  return (
    <div
      className={cn(
        "flex flex-col justify-between w-full h-full space-y-4 bg-neutral-lightest rounded-lg lg:p-6 p-4",
        className
      )}
    >
      {children}
...
export function FacilityIcon({ children }: FacilityIconProps) {
  return (
    <div className="flex items-center justify-start w-full">{children}</div>
  );
}
